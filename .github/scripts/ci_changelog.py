...
def parse_diff(diff_output):
    ...
    return model_changes, changes


def format_changelog(model_changes, term_changes):
    """Format changes into markdown."""
    lines = []

    for collection, action in model_changes:
        lines.append(f"- {collection}: {action} data descriptor model")
...
def main():
    ...
    model_changes, term_changes = parse_diff(diff_output)
    print(format_changelog(model_changes, term_changes))
